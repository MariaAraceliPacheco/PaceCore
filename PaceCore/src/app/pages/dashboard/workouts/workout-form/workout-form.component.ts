...
import { IntervalCreate } from '../../../../../models/intervalos/interval-create.model';
...
export class WorkoutFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    console.log(
      'WorkoutFormComponent inicializando, suscribiéndose a actividades...',
    );
    this.workoutService.actividades$.subscribe({
      ...
      error: (err) =>
        console.error('Error en la suscripción de actividades:', err),
    });
    this.initForm();
  ...
  onSubmit(): void {
    if (this.workoutForm.valid) {
      ...
      const mappedIntervalos: IntervalCreate[] = formData.intervalos.map(
        (interval: any) => {
          const intervalData = {
      ...
      if (this.entreno) {
        ...
        this.workoutService.addWorkout(workoutData).subscribe({
          ...
          error: (err) => {
            console.error('Error añadiendo entrenamiento:', err);
            this.snackBar.open('Error al añadir el entrenamiento', 'Cerrar', {
              duration: 5000,
            });
          },
        });
  ...
  private patchFormValues(): void {
    ...
    this.workoutForm.patchValue({
      ...
      tiempo_total: this.formatTimeObject(
        this.entreno.tiempo_total_entreno || (this.entreno as any).tiempoTotal,
      ),
      tipo_actividad_id:
        this.entreno.tipo_actividad_entreno?.id ||
        (this.entreno as any).tipoactividad?.id,
      desnivel:
        this.entreno.desnivel_entreno !== undefined
          ? this.entreno.desnivel_entreno
          : (this.entreno as any).desnivel,
      fcMedia:
        this.entreno.fcMedia !== undefined
          ? this.entreno.fcMedia
          : (this.entreno as any).fc_media_entreno,
      fcMaxima:
        this.entreno.fcMaxima !== undefined
          ? this.entreno.fcMaxima
          : (this.entreno as any).fc_max_entreno,
    ...
    if (this.entreno.intervalos && this.entreno.intervalos.length > 0) {
      console.log(
        'DEBUG: Cargando intervalos del entreno:',
        this.entreno.intervalos,
      );
      this.entreno.intervalos.forEach((intervalo, index) => {
        ...
        const activityId =
          (intervalo as any).tipo_actividad_id?.id ||
          (intervalo as any).tipo_actividad?.id;
        ...
        const intervalForm = this.fb.group({
          ...
          fcMedia: [
            intervalo.fcMedia !== undefined
              ? intervalo.fcMedia
              : (intervalo as any).fc_media,
          ],
          fcMaxima: [
            intervalo.fcMaxima !== undefined
              ? intervalo.fcMaxima
              : (intervalo as any).fc_maxima,
          ],
        });
        this.intervalos.push(intervalForm);
      });
  ...
  private calculateTotalsFromIntervals(): void {
    ...
    this.workoutForm.patchValue(
      ...
      { emitEvent: false },
    ); // Evitar bucle infinito de ValueChanges
  }
